...
class SensorLogic(object):
    ...
    def get_data_for_day(cls, sensor_id, year, month, day):
        query = 'SELECT "Avg", "Minute" FROM (SELECT avg(depth) As "Avg", date_trunc(\'minute\', date) AS "Day" FROM measurement WHERE sensor_id = :sensor_id AND (TIMESTAMP \'{year}-{month}-{day} 00:00:00\', TIMESTAMP \'{year}-{month}-{day} 24:00:00\') OVERLAPS (date, date) GROUP BY "Day") AS "Data" RIGHT JOIN (SELECT generate_series( TIMESTAMP\'{year}-{month}-{day} 00:00\' , TIMESTAMP\'{year}-{month}-{day} 23:59\', \'1 minute\') as "Minute") AS "Hours" ON "Hours"."Minute" = "Data"."Day" ORDER BY "Hours"."Minute"'.format(year=year+0, month=month+0, day=day+0)
